...
$U\times_WV = U\times_ZY \intersect X\times_ZV$. If $\phi: T \to X\times_ZY$ then $\phi^{-1}(U\times_WV)=\phi^{-1}(U\times_ZY
    ) \intersect \phi^{-1}(X\times_ZV)$.
Clearly also $\phi^{-1}(U\times_ZY)=(\pi_X \after
    \phi)^{-1}(U)$.
